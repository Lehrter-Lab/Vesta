...
def read_points(infile):
    ...
    # For .h5 file generated by SMS from scatter
        ...
        with h5py.File(infile, 'r') as f:
            dataset_path = None
            def match_coords(name, obj):
                nonlocal dataset_path
                if (
                    isinstance(obj, h5py.Dataset)
                    and "NodeLocs" in name
                    and obj.ndim == 2
                    and obj.shape[1] >= 2
                ):
                    dataset_path = name

            f.visititems(match_coords)

            if not dataset_path:
                raise KeyError("Could not find a suitable coordinate dataset (containing 'NodeLocs').")

            coords = f[dataset_path][:]
            xs = coords[:, 0]
            ys = coords[:, 1]
