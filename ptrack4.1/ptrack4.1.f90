...
!										
!	SCHISM Particle tracking code for _scribed_ nc outputs. Works for mixed
...
!          d) out2d*.nc, horizontalVel*.nc, verticalVelocity*.nc (also diffusivity and wind for oil spill)
...
!...  Main program
      ...
      character(len=30) :: file63,file64,file65,file66,file67
...
!      if(iret/=nf90_NoErr) stop 'w not found'
      if(mod_part==1) then
        iret=nf90_inq_varid(ncid,'windSpeedX',lwindx)
        if(iret/=nf90_NoErr) stop 'windx not found'
        iret=nf90_inq_varid(ncid,'windSpeedY',lwindy)
...
!...  Read in elevation and vel. info
      ...
      eta2=real_ar(1,1:np)
      iret=nf90_get_var(ncid2,lu_id,real_ar(1:nvrt,1:np),(/1,1,irec1/),(/nvrt,np,1/))
      if(iret/=nf90_NoErr) stop 'uu2 not read'
      uu2(:,:)=transpose(real_ar(1:nvrt,1:np))
      iret=nf90_get_var(ncid3,lv_id,real_ar(1:nvrt,1:np),(/1,1,irec1/),(/nvrt,np,1/))
      ...
      if(mod_part==1) then
        ...
        iret=nf90_get_var(ncid,lwindy,wny2(1:np),(/1,irec1/),(/np,1/))
        if(iret/=nf90_NoErr) stop 'windx not read'
!        wnx2(:)=real_ar(1,1:np)
!        wny2(:)=real_ar(2,1:np)
        iret=nf90_get_var(ncid5,ltdff,real_ar(1:nvrt,1:np),(/1,1,irec1/),(/nvrt,np,1/))
...
449     continue
        ...
        write(95,'(i12,1x,i12,2(1x,e22.14),1x,f12.3)')i,ielpar(i),xout,yout,zpar(i)-eta_p
